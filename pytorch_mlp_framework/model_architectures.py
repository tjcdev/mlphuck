...
class ConvolutionalDimensionalityReductionBlock(nn.Module):
    ...
    def build_module(self):
        ...
        # Random zeros @20%    
       # self.layer_dict['drop'] = nn.Dropout(0.2)
       # out = self.layer_dict['drop'].forward(out)
        ...
        #downsampling
       # self.layer_dict['downsample'] = nn.Conv2d(in_channels=x.shape[1], out_channels=out.shape[1], bias=self.bias, kernel_size=self.kernel_size, dilation=self.dilation, padding=self.padding, stride=2)
